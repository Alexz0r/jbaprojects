...
def number_of_monthly_payments(l_principal, month_payment, ln_interest):
    ...
    periods = math.ceil(math.log((month_payment / (month_payment - nominal_interest_rate * l_principal)),
                                 1 + nominal_interest_rate))
    result_str = f"It will take {(periods_to_year(periods))} to repay this loan!"
...
def monthly_payment(payment_type, l_principal, periods, ln_interest):
    ...
    if payment_type == "annuity":
        month_payment = math.ceil(l_principal * nominal_interest_rate * pow(1 + nominal_interest_rate, periods) /
                                  (pow(1 + nominal_interest_rate,  periods) - 1))
        result_str = f"Your monthly payment = {month_payment}!"
        total_sum = periods * month_payment

    elif payment_type == "diff":
        for i in range(periods):
            month_payment = math.ceil(l_principal / periods + nominal_interest_rate *
                                      (l_principal - l_principal * (i + 1 - 1) / periods))
            total_sum += month_payment
            result_str = result_str + f"Month {i+1}: payment is {month_payment}\n"
    result_str += f"\nOverpayment = {int(total_sum - l_principal)}"
    return result_str
...
def loan_principal(payment, periods, ln_interest):
    nominal_interest_rate = ln_interest / (12 * 100)
    l_principal = math.floor(payment / (nominal_interest_rate * pow(1 + nominal_interest_rate, periods) /
                                   (pow(1 + nominal_interest_rate, periods) - 1)))
    result_str = f"Your loan principal = {l_principal}!"
